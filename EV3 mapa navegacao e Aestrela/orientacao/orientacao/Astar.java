...
public class Astar {
	...
	public List<Celula> search(Posicao inicio, Posicao fim) throws Exception {
		...
		listaAberta.add(mapa[inicio.getX()][inicio.getY()]);
		...
		while (listaAberta.size() > 0) {
			...
			//verifica se a celula com o menor f da lista aberta é a celula de fim
			// Fim -- o resultado foi encontrado, retorne o caminho encontrado
			if (celulaAtualfMenor == mapa[fim.getX()][fim.getY()]) {
			...
			// celulaAtualfMenos nao é a celula de fim 
			...
			for (int i = 0; i < vizinhos.size(); i++) {
				...
				// que vimos até agora
				int pontoG = celulaAtualfMenor.g + 1; // 1 é a distancia do nó até o seu vizinho
	...
	public List<Celula> searchReversePath(Posicao inicio, Posicao fim) throws Exception{
		...
		while (listaAberta.size() > 0) {
			...
			// Fim -- o resultado foi encontrado, retorne o caminho encontrado
			if (noAtual == mapa[fim.getX()][fim.getY()]) {
	...
	private List<Celula> vizinhos(Celula no) {
		...
		if(posicao.getX()>0){
			ret.add(mapa[posicao.getX() - 1][posicao.getY()]);
		}
		if(posicao.getX()<mapa.length-1){
			ret.add(mapa[posicao.getX() + 1][posicao.getY()]);
		}
		if(posicao.getY()>0){
			ret.add(mapa[posicao.getX()][posicao.getY() - 1]);

		}
		if(posicao.getY()<mapa.length-1){
			ret.add(mapa[posicao.getX()][posicao.getY() + 1]);
		}
		return ret;
	}
	...
	private int heuristica(Posicao p0, Posicao p1) {
		int d1 = Math.abs(p1.getX() - p0.getX());
		int d2 = Math.abs(p1.getY() - p0.getY());
		return d1 + d2;
	}
